...
CUDA_VISIBLE_DEVICES=0 sudo /home/raghu/raghu/bgnenv/bin/python3.10 1a_Noise_type_identification_LFCC_13102025_enhancement_ACC_consistency.py --layers 6 --epochs 50 --specaugment
...
# ==========================
# LFCC Extraction Function
# ==========================

def make_linear_filterbank(sr, n_fft, n_bands):
    """
    Create a linearly spaced triangular filterbank (like mel, but linear).
    """
    f_min, f_max = 0, sr / 2
    freqs = np.linspace(f_min, f_max, n_bands + 2)  # equally spaced on linear scale
...
def lfcc_extraction(waveform, sample_rate=8000, n_fft=512, hop_length=256, n_lfcc=13, n_filter=40):
    """
    ...
    # Linear filterbank
    linear_fb = make_linear_filterbank(sample_rate, n_fft, n_filter)

    # Apply filterbank
    linear_s = np.dot(linear_fb, S[:linear_fb.shape[1], :])

    # Log compression
    log_linear = np.log(linear_s + 1e-10)

    # DCT → LFCC
    lfcc = librosa.feature.mfcc(S=log_linear, n_mfcc=n_lfcc)

    return torch.tensor(lfcc, dtype=torch.float32)
...
def plot_representations(features, labels, preds, method="tsne", color_by="labels"):
    ...
    Args:
        features: np.ndarray, extracted hidden features
        labels: np.ndarray, true labels
        preds: np.ndarray, predicted labels
        method: "tsne" or "pca"
        color_by: "labels" (default) to color by true labels, 
                  "preds" to color by predicted labels
    """
    if method == "tsne":
...
# ==========================
# Custom Dataset with SpecAugment
# ==========================
...
# ==========================
# Flexible CNN Model with Safer Pooling
# ==========================
# ==========================
...
class NoiseClassifierCNN(nn.Module):
    def __init__(self, num_classes=4, num_conv_layers=6):
        ...
        for i, out_channels in enumerate(channels):
            kernel_size = 5 if i == 0 else 3  # Larger kernel for first layer
        ...
        self.se_blocks = nn.ModuleList([SEBlock(ch) for ch in channels])  # Add SE after each conv block
...
def audio_transform(waveform):
    lfcc = lfcc_extraction(waveform, sample_rate=8000, n_fft=512, hop_length=256, n_lfcc=40, n_filter=60)
    return lfcc.unsqueeze(0)  # Add channel dim for CNN



# ==========================
# Training Script (unchanged, uses audio_transform now)
# ==========================
def main(args):
    ...
    csv_path = os.path.join(save_dir, f"metrics_LFCC_{args.layers}layers_{specaug_tag}_{current_date}.csv")
    pdf_path = os.path.join(save_dir, f"report_LFCC_{args.layers}layers_{specaug_tag}_{current_date}.pdf")
    view_path = os.path.join(save_dir, f"classification_view_LFCC_{args.layers}layers_{specaug_tag}_{current_date}.csv")
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(save_dir, f"noise_classifier_LFCC_{args.layers}layers_best.pth"))
        else:
            patience_counter += 1
    ...
    # (report saving section unchanged...)
    # -----------------------
    # keep the same CSV, PDF, visualization code
    # -----------------------
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Noise Type Identification with CNN + SpecAugment toggle (LFCC version)")
