...
CUDA_VISIBLE_DEVICES=0 sudo /home/raghu/raghu/bgnenv/bin/python3.10 1_Noise_type_identification_MFCC_updated.py --layers 6 --epochs 50 --specaugment
'''
...
def mfcc_extraction(waveform, sample_rate=8000, n_mfcc=40, n_fft=512, hop_length=256):
    """Extract MFCCs using librosa from a torch waveform tensor.
    Returns a torch.FloatTensor of shape (n_mfcc, time_frames)
    """
...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            if features.dim() > 3:
                features = features.unsqueeze(0) if features.dim() == 2 else features.squeeze(0)
...
# ==========================
# Enhanced CNN Model (from LFCC flow)
# ==========================
...
class NoiseClassifierCNN(nn.Module):
    ...
    def forward(self, x, return_features=False):
        if x.dim() == 3:
            # input shape expected: (batch, n_mfcc, time) -> add channel dim
            x = x.unsqueeze(1)
        if x.dim() == 5:
            x = x.squeeze(2)
...
# ==========================
# audio_transform using MFCCs
# ==========================

def audio_transform(waveform):
    mfcc = mfcc_extraction(waveform, sample_rate=8000, n_mfcc=40, n_fft=512, hop_length=256)
    # Add channel dim for CNN: (n_mfcc, time) -> (1, n_mfcc, time)
    return mfcc
...
def main(args):
    ...
    csv_path = os.path.join(save_dir, f"metrics_MFCC_{args.layers}layers_{specaug_tag}_{current_date}.csv")
    pdf_path = os.path.join(save_dir, f"report_MFCC_{args.layers}layers_{specaug_tag}_{current_date}.pdf")
    view_path = os.path.join(save_dir, f"classification_view_MFCC_{args.layers}layers_{specaug_tag}_{current_date}.csv")
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(save_dir, f"noise_classifier_MFCC_{args.layers}layers_best.pth"))
        else:
            patience_counter += 1
    ...
    # If best_probs is empty (no validation), fallback to last epoch
    ...
    summary = {"epoch": "final", "train_loss": np.min(history["train_loss"]) if len(history["train_loss"])>0 else np.nan,
               "val_loss": np.min(history["val_loss"]) if len(history["val_loss"])>0 else np.nan, "val_accuracy": best_val_acc,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Noise Type Identification with CNN + SpecAugment toggle (MFCC version)")
