...
Example run:
source /home/raghu/raghu/bgnenv/bin/activate
CUDA_VISIBLE_DEVICES=0 sudo /home/raghu/raghu/bgnenv/bin/python3.10 Noise_type_identification_HFCC_16092025_PDF_model_Building_randonseed_131012025(1).py --layers 6 --epochs 50 --specaugment
'''
import os
import argparse
import random
...
# ==========================
# Set Random Seed for Reproducibility (match LFCC script)
# ==========================
...
# ==========================
noise_types = ['Car', 'Exhibition', 'Station']   #Babble omitted intentionally
...
# ==========================
# HFCC Extraction Function (kept but adjusted to be robust like LFCC)
# ==========================
def hz_to_erb(f):
    """Convert frequency (Hz) to ERB number."""
    return 21.4 * np.log10(1 + 0.00437 * f)

def erb_to_hz(erb):
    """Convert ERB number back to Hz."""
    return (10**(erb / 21.4) - 1) / 0.00437

def make_erb_filterbank(sr, n_fft, n_bands):
    """Create ERB-scaled triangular filterbank."""
    f_min, f_max = 0, sr / 2
...
def hfcc_extraction(waveform, sample_rate=8000, n_fft=512, hop_length=256, n_hfcc=40):
    """
    Extract HFCC features using a manually constructed ERB filterbank.
    Returns tensor shape (n_hfcc, time_frames)
    """
    y = waveform.squeeze().detach().cpu().numpy()
    S = np.abs(librosa.stft(y, n_fft=n_fft, hop_length=hop_length))**2
    erb_fb = make_erb_filterbank(sample_rate, n_fft, n_hfcc)
    # Adjust S shape to match filterbank columns if needed
    frame_len = S.shape[0]
    fb_cols = erb_fb.shape[1]
    cols = min(frame_len, fb_cols)
    erb_s = np.dot(erb_fb[:, :cols], S[:cols, :])
...
# ==========================
# Custom Dataset with deterministic SpecAugment
# ==========================
class AudioDataset(Dataset):
    def __init__(self, file_paths, labels, transform=None, augment=False, target_length=16128, use_specaugment=True):
        ...
        # For pitch shift usage when augment=True
        ...
        if augment and use_specaugment:
            ...
            self.freq_mask_param = 13
        else:
            self.time_mask_param = None
            self.freq_mask_param = None
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            if self.augment and self.use_specaugment:
                # Deterministic specaugment masks based on sample idx
                rng = np.random.default_rng(42 + idx)
                tm = int(rng.integers(0, max(1, self.time_mask_param or 1)))
                fm = int(rng.integers(0, max(1, self.freq_mask_param or 1)))
                if self.time_mask_param:
                    features = transforms.TimeMasking(time_mask_param=tm)(features)
                if self.freq_mask_param:
                    features = transforms.FrequencyMasking(freq_mask_param=fm)(features)
        else:
            features = waveform
...
# ==========================
# Enhanced CNN Model (SE + Residual) - same structure as LFCC script
# ==========================
class SEBlock(nn.Module):
    def __init__(self, channel, reduction=8):
        ...
        self.se = nn.Sequential(
            ...
            nn.Conv2d(channel, max(channel // reduction, 1), kernel_size=1),
            nn.ReLU(),
            nn.Conv2d(max(channel // reduction, 1), channel, kernel_size=1),
            nn.Sigmoid()
        )
    def forward(self, x):
        return x * self.se(x)
...
def audio_transform(waveform):
    # Use a more compact HFCC (like prior version) but keep shape robust
    hfcc = hfcc_extraction(waveform, sample_rate=8000, n_fft=512, hop_length=256, n_hfcc=40)
    return hfcc.unsqueeze(0)  # channel dim
...
def main(args):
    ...
    # Ensure reproducibility every run
    set_seed(42)
    ...
    csv_path = os.path.join(save_dir, f"metrics_HFCC_{args.layers}layers_{specaug_tag}_{current_date}.csv")
    pdf_path = os.path.join(save_dir, f"report_HFCC_{args.layers}layers_{specaug_tag}_{current_date}.pdf")
    view_path = os.path.join(save_dir, f"classification_view_HFCC_{args.layers}layers_{specaug_tag}_{current_date}.csv")
    ...
    # Deterministic DataLoader seeds
    g = torch.Generator()
    g.manual_seed(42)
    ...
    # store best epoch predictions/probs
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(save_dir, f"noise_classifier_HFCC_{args.layers}layers_best.pth"))
        else:
            patience_counter += 1
    ...
    for i, noise in enumerate(noise_types):
        try:
            ...
            auc_scores[noise] = np.nan
            calib_errors[noise] = np.nan

    summary = {"epoch": "final", "train_loss": np.min(history["train_loss"]) if history["train_loss"] else np.nan,
               "val_loss": np.min(history["val_loss"]) if history["val_loss"] else np.nan,
               "val_accuracy": best_val_acc, "SpecAugment": args.specaugment}
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        plt.figure()
        for i, noise in enumerate(noise_types):
            if not np.isnan(auc_scores[noise]):
                fpr, tpr, _ = roc_curve(all_labels_bin[:, i], all_probs[:, i])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Noise Type Identification with CNN + SpecAugment toggle (HFCC version)")
