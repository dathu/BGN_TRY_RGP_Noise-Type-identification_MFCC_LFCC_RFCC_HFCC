'''
sudo /home/raghu/raghu/bgnenv/bin/python3.10 2a_Noise_type_identification_24092025_LFCC_model_load_benchmark_latest.py \
  --data_dir noisezus_30speakers \
  --model_path noise_classifier_LFCC_6layers_best.pth \
  --layers 6 \
  --batch_size 16
'''
import os
...
# =====================================================
# LFCC feature extraction (from training script)
# =====================================================
def make_linear_filterbank(sr, n_fft, n_bands):
    f_min, f_max = 0, sr/2
    freqs = np.linspace(f_min, f_max, n_bands + 2)
...
def lfcc_extraction(waveform, sample_rate=8000, n_fft=512, hop_length=256,
                    n_lfcc=40, n_filter=60):
...
def audio_transform(waveform):
    return lfcc_extraction(waveform).unsqueeze(0)  # Add channel dimension
...
# =====================================================
# Model (must match LFCC training)
# =====================================================
...
def main(args):
    ...
    save_dir = "./"
    csv_path = os.path.join(save_dir, "benchmark_metrics_LFCC.csv")
    pdf_path = os.path.join(save_dir, "benchmark_report_LFCC.pdf")
    view_path = os.path.join(save_dir, "benchmark_classification_LFCC.csv")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Benchmark pre-trained LFCC noise classifier")
    parser.add_argument("--data_dir", type=str, default="noisezus_30speakers")
    parser.add_argument("--batch_size", type=int, default=16)
    parser.add_argument("--model_path", type=str, default="noise_classifier_6layers_best_LFCC.pth")
